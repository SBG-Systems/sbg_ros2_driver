...
class TestSbgDeviceShutdown(unittest.TestCase):
    ...
    def test_shutdown_is_clean(self, proc_info):
        ...
        # node only returns from sbgEComHandle() once the recording is exhausted, which the
        # four second fixture guarantees well inside the shutdown grace period, so it reaches
        # its signal handling and leaves its main loop. The signal exit codes are accepted as
        # well in case it is terminated before it gets there.
        #
