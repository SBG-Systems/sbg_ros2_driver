...
void ConfigStore::loadInsParameters(const rclcpp::Node& ref_node_handle)
{
  ref_node_handle.get_parameter_or<std::string>("ins.settingsFile", ins_settings_file_, "");

  //
  // The presence of a device configuration parameter tells that the configuration is
  // still using the deprecated sbgECom command path.
  //
  legacy_params_present_ = ref_node_handle.has_parameter("sensorParameters.motionProfile");
}
...
void ConfigStore::loadDeprecatedInsParameters(const rclcpp::Node& ref_node_handle)
{
...
const std::string &ConfigStore::getInsSettingsFile() const
{
  return ins_settings_file_;
}

bool ConfigStore::hasLegacyInsParameters() const
{
  return legacy_params_present_;
}
...
void ConfigStore::loadFromRosNodeHandle(const rclcpp::Node& ref_node_handle)
{
  ...
  loadInsParameters(ref_node_handle);
...
#ifdef SBG_USE_DEPRECATED_ECOM_CONFIG
  loadDeprecatedInsParameters(ref_node_handle);
#endif
}
